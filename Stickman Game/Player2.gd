...
#	if Input.is_action_pressed("move_right_%s" % [player_id]): movement.x -= 1.0 
	...
	if Input.is_action_just_pressed("move_right_2"): movement.x -= 1.0 
	if Input.is_action_just_pressed("move_right_2"): anim.play("Jump2")
	
	if Input.is_action_just_released("move_right_2"): anim.play("Idle2")


#else: anim.play("Idle")


#	if Input.is_action_pressed("move_left_%s" % [player_id]): movement.x += 1.0
